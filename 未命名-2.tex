...
\fbox{
    \begin{minipage}{30em}%
     ...
     \begin{center}
        \includegraphics{picture/1-7.png}
     \end{center}
        \solutionii{}
    \end{minipage} 
}
...
\fbox{
    \begin{minipage}{30em}%
        ...
        \solutioni{体心立方晶体固体物理学原胞相对应的三个基矢以及相应原胞的体积分别为：\[\left\{\begin{array}{l}
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{若某晶体的基矢为$\vec{a}_1=a\vec{i},\vec{a}_2=a\vec{j},\vec{a}_3=\frac{a}{2}(\vec{i}+\vec{j}+\vec{k})$, 试根据给定的基判断该晶体具有何种晶体结构?}
        \solutioni{体心立方。}
    \end{minipage}
}
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{试画出体心立方和面心立方晶格结构的晶体在（100），（110）和（111）晶面上的原子排列。}
        \solutioni{略}
    \end{minipage}
}
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{对如图所示的三维布喇菲格子，原胞的基矢方向如图所示，试求：\newline 
        （1）OA 晶列和 CA 晶列的晶向指数；\newline 
        （2）BCD 晶面和 ABC 晶面的面指数。}
        \begin{center}
            \includegraphics{picture/2-15.png}
        \end{center}
        \solutioni{（1）OA 晶列$[322]$  CA 晶列$[302]$\newline
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{ 考虑一个面心立方结构的晶体，其晶胞及基矢如图所示，A 是其中一个顶 
        角原子，B、C 是晶胞上的两个面心原子， 求：\newline（1）AC 晶列的晶向指数；\newline 
        （2）ABC 晶面的密勒指数。 }
        \begin{center}
            \includegraphics{picture/2-16.png}
        \end{center}
        \solutioni{（1）AC 晶列的晶向指数$[\bar{1}\bar{1}2]$\newline
...
\fbox{
    \begin{minipage}{30em}%
        \problemii{已知三斜晶系的晶体中，三个基矢分别为$\vec{a}_1,\vec{a}_2 \text{和} \vec{a}_3$。现测知该晶体的 
        某一晶面法线与基矢的夹角依次为$\alpha,\beta \text{和} \gamma$，试求该晶面的面指数。}
        \solutionii{\[(\dfrac{1}{|\vec{a}_1 \cos \alpha|},\dfrac{1}{|\vec{a}_2 \cos \beta|},\dfrac{1}{|\vec{a}_3 \cos \gamma|})\]}
    \end{minipage}
}

\fbox{
    \begin{minipage}{30em}%
        \problemii{对二维晶体，试论证二维平面点阵不可能有 5 度旋转对称轴。}
        \solutionii{略}
    \end{minipage}
}
...
\fbox{
    \begin{minipage}{30em}%
        \problemii{石墨烯具有下图所示的正六角网状的结构形式，试问石墨烯的这种平面网 
        状格子是简单布喇菲格子还是复式格子？为什么？试画出固体物理学原胞和晶 
        胞。 }
        \begin{center}
            \includegraphics{picture/2-19-1.png}
        \end{center}
        \solutionii{石墨烯的这种平面网状格子是复式格子，若设石墨烯中相邻碳原子是等价的，会发现沿着A-B原子延伸出去1个单位的晶格平移矢量的地方其实并没有原子存在。也就说相邻的碳原子不等价，因为所处的环境并不相同。}
        \begin{center}
            \includegraphics{picture/2-19-2.png}
        \end{center}
    \end{minipage}
}
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{试思考：从正格子空间到与之对应的傅里叶空间的变换，为什么不能采取通 
        常的坐标系变换的方式？}
        \solutioni{晶体具有破缺的平移对称性，因此，对晶体而言，当从真实空间变换到与之对应的傅里叶空间时，其变换应不同于通常的傅里叶变换，或者说，在对晶体进行傅里叶变换时需要考虑晶体平移对称性破缺的事实。}
    \end{minipage}
}

\fbox{
    \begin{minipage}{30em}%
        \problemi{试思考：为什么说晶体中物理性质的平移对称性是破缺的？ }
        \solutioni{略。}
    \end{minipage}
}
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{试思考：如何从正、倒格子不同角度来理解晶体结构的几何特征？}
        \solutioni{略。}
    \end{minipage}
}
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{试思考：如果$K_{h_1 h_2 h_3}=h_1\vec{b}_1+h_2\vec{b}_2+h_3\vec{b}_3$是任意的倒格矢，则该倒格矢的长度与晶面指数为$(h_1,h_2,h_3)$的晶面族面间距间的关系如何？}
        \solutioni{\[\begin{gathered}
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{试证明：简单六角布喇菲格子的倒格子仍为简单六角布喇菲格子，并给出其 
        倒格子空间的晶格常数。 }
        \begin{center}
            \includegraphics{picture/3-5.png}
        \end{center}
        \solutioni{六角晶系晶胞示意图, 其中$a=b\ne c ,\alpha=\beta=90^o$和$\gamma=120^o$
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{试证明：体心立方晶格的倒格子是具有面心立方结构的格子，而面心立方晶 
        格的倒格子是具有体心立方结构的格子。}
        \solutioni{例如, 对晶格常数为  的体心立方格子, 根据固体物理学原胞的习惯选取方法,正格子空间的三个基矢在直角坐标系中分别表示为:$$
        \left\{\begin{array}{l}
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{试论证二维晶体的倒格子原胞面积和正格子原胞面积间的关系。 }
        \solutioni{\[\left\{\begin{array}{l}
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{ 试证明正格子空间中一族晶面$(h_1h_2h_3)$和倒格矢$\vec{K}_{h}$正交。}
        \solutioni{\[
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{如果基矢$\vec{a},\vec{b},\vec{c}$构成简单正交系，试证明晶面族$(hkl)$的面间距为：$$
        d_{h k l}=\frac{1}{\sqrt{\left(\frac{h}{a}\right)^{2}+\left(\frac{k}{b}\right)^{2}+\left(\frac{l}{c}\right)^{2}}}
        $$并说明面指数简单的晶面, 其面密度比较大, 容易解理。}
        \solutioni{简单正交杀 $\vec{a} \perp \vec{b} \perp \vec{c}$ $ \vec{a}_{1}=a\vec{i}, \vec{a}_{2}=b \vec{j}, \vec{a}_3=c\vec{k}$
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{对晶格常数为$a$和$b$的二维矩形格子，试证明相应的倒格子仍然为二维矩形 
...
\fbox{
    \begin{minipage}{30em}%
        \problemi{试问为什么面心立方晶体的第一布里渊区不是一个正八面体而是一个截角 
        的八面体？}
        \begin{center}
            \includegraphics{picture/3-12.png}
        \end{center}
        \solutioni{对具有体心立方结构的倒格子, 每个倒格点同围有8个近邻倒格点。若选取立方体中心的倒格点作为原点$\Gamma$, 则在如图所示的直角坐标系中, 这8个近 邻倒格点的倒格矢分别为:
        \[\begin{aligned}
