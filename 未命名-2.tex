...
\SetWatermarkText{ZhuangZhuang Lu}           % 设置水印内容
%\SetWatermarkText{\includegraphics{fig/texlion.png}}         % 设置水印logo
\SetWatermarkLightness{0.92}             % 设置水印透明度 0-1
\SetWatermarkScale{0.6}                   % 设置水印大小 0-1  
%页眉页脚
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\color{blue}by ZhuangZhuang Lu}
\fancyhead[R]{\color[rgb]{0,0,1}\textrm{\thepage}}
\fancyfoot[C]{\color{green}理科的脸都被物理丢尽了,尤其是那个傻逼物理实验.}
%
...
\maketitle
\begin{center}
{\color{red}注:黑色字体为题目,红色为其解答；作业之外习题,蓝色为其解答.}
\end{center}
{\color{blue}\tableofcontents}

\section{第一章}

\proi{试思考:为什么说电磁力应是引起原子凝聚的吸引力的来源?}{弱相互
作用和强相互作用均是短程相互作用,其作用范围在$~10^{-15}-10^{-13}
cm$,这比原子自身的尺寸$(~10^{-8}cm)$还小得多,因此,应当排除弱相
互作用和强相互作用作为引起原子凝聚的吸引力的来源.
引力和电磁力属长
程相互作用,毫无疑问,原子之间存在引力,但引力太弱,若以强相互作用为标
准,则电磁力、弱相互作用力和引力依次在$ 10^{-2}、10^{-12} \text{和} 10^{-40}$的量级.
综合这些考虑,可以认为,电磁力应是引起原子凝聚的吸引力的来源.
}
     
\proi{试思考:如果说带电粒子间的库仑作用应是产生原子间相互作用力
 的物理根源,那不带电粒子靠什么力能吸引到一起?}{略}
    
\proi{试思考:在五种基本结合力中,自然界中哪种结合力最普遍?}{略}

\proi{试思考:为什么金属键和离子键没有方向性而共价键和氢键有方向性?}{略}
    
\proi{试思考:$KBr$、$CCl_4$和$H_2S$分别属于何种类型的化合物? }{略}
    
\proi{试思考:$BeH_2$、$BBr_3$和$SiH_4$三种化合物中原子有哪种可
能的杂化类型?能预测出它们的几何构型吗?}{略}
   
\proii{考虑由正、负两种离子沿$x$和$y$轴方向周期性排布构成如图所示的二维离子 
     固体,假设两个方向上的周期(相邻正、负离子间的间隔)均为$a$,若只考 
     虑边长为$2a$的正方格子中的正、负离子,试计算其马德隆常数的近似值.} 
[\begin{center} \includegraphics{picture/1-7.png} \end{center} ]{\[M=\dfrac{1}{2} \times 2 -\dfrac{1}{\sqrt{2}}-\dfrac{1}{4}
\times 2 \times \dfrac{1}{2}+\dfrac{1}{2} \times 2 \times \dfrac{1}{\sqrt{5}}-\dfrac{1}{4} \times 1 \times \dfrac{1}{2\sqrt{2}}\]
\[M \approx 0.4017185 \]
}
...
\proi{假设一固体中原子间的相互作用能可表示为:$U(r)=-\dfrac{\alpha}{r^m}+\dfrac{\beta}{r^n}$,试计算:\newline
        ...
        多少?}{(1)\[r_0=\left(\dfrac{m\alpha}{n\beta}\right)^{\dfrac{1}{m-n}}\]
        (2)\[W=-\dfrac{1}{2}U(r_0)=\dfrac{1}{2}\alpha\left(1-\dfrac{m}{n}\right)\left(\dfrac{n\beta}{m\alpha}\right)^{\dfrac{m}{m-n}}\]
        (3)体弹性模量:\[K=\dfrac{nm|U_0|}{9V_0}\]
        \[V_0=N\beta r_0^3,U_0=U(r_0)\]
        (4)由(1),(2)得:\[\beta=\dfrac{W}{2}=r_0^{10}=1.18 \times 10^{-95}eV \cdot m^{10}\]
        \[\alpha=r_0^2\left[\dfrac{\beta}{r_0^{10}}+2W\right]=9.0\times 10^{-19}eV \cdot m^2\]}

\proi{设一固体平衡时体积为$V_0$,原子间总的相互作用能为$U_0$,如果原子间
    相互作用能由式$U(r)=-\dfrac{\alpha}{r^m}+\dfrac{\beta}{r^n}$所表述,
    试证明体弹性模量为$\dfrac{nm|U_0|}{9V_0}$.}{同上题}
...
\proi{对由N个正、负离子构成的离子固体,假设其结合能为\[U(r)=\dfrac{N}{2}(-\dfrac{\alpha e^2}{4\pi \varepsilon_0 r}+\dfrac{\beta}{r^n})\]
    ...
    如果这两者对互作用势能的贡献相同,试求$n$和$\rho$之间的关系.}{\[\dfrac{\beta}{r_0^n}=ce^{\dfrac{r_0}{\rho}}\]
    \[\dfrac{n\beta}{r_0^{n+1}}=\dfrac{c}{\rho}e^{-\dfrac{r_0}{\rho}}\]
    \[r_0=n\rho , \rho=\dfrac{e}{n}\left(\dfrac{\beta}{c}\right)^{\dfrac{1}{n}}\]}
    
\proii{针对金刚石、石墨、碳60、碳纳米管和石墨烯五种碳同素异构体,虽然它 
    们均是由纯碳原子构成的,试分析为什么彼此间性能却有非常大的差别?}{由于 C
    原子间成键方式的不同，这些碳同素异构体具有不同的空间结构形式并表现出不
    同甚至完全不同的力学性能、物理性质等.}
...
\proi{对于六角密堆积结构,试证明:$\dfrac{c}{a}=(\dfrac{8}{3})^{1/2}$,如果明显大于此值,则可能发生何种现象?}
...
\proi{对体心立方和面心立方结构的晶体,其固体物理学原胞的习惯选取如图2.23所示,试证明:对体心立方晶体其原胞三基矢间的夹角为$109^o27'$,对面心立方晶体其原胞三基矢间的夹角为$60^o$.}
...
\proi{若某晶体的基矢为$\vec{a}_1=a\vec{i},\vec{a}_2=a\vec{j},\vec{a}_3=\dfrac{a}{2}(\vec{i}+\vec{j}+\vec{k})$,试根据给定的基判断该晶体具有何种晶体结构?}
    {体心立方.}
...
\proi{试思考:如果$K_{h_1 h_2 h_3}=h_1\vec{b}_1+h_2\vec{b}_2+h_3\vec{b}_3$是任意的倒格矢,则该倒格矢的长度与晶面指数为$(h_1,h_2,h_3)$的晶面族面间距间的关系如何?}
    {\[\begin{gathered}
     d_{h_{1} h_{2} h_{3}}=\dfrac{\vec{a}_{1}}{h_{1}} \cdot \vec{e}_{n}=\dfrac{\vec{a}_{1}}{h_{1}} \cdot \dfrac{\vec{K}_{h_{1} h_{2} h_{3}}}{\left|\vec{K}_{h_{1} h_{2} h_{3}}\right|}=\dfrac{\vec{a}_{1}}{h_{1}} \cdot \dfrac{h_{1} \vec{b}_{1}+h_{2} \vec{b}_{2}+h_{3} \vec{b}_{3}}{\left|\vec{K}_{h_{1} h_{2} h_{3}}\right|} \\
     d_{h_{1} h_{2} h_{3}}=\dfrac{2 \pi}{\left|\vec{K}_{h_{1} h_{2} h_{3}}\right|}
    \end{gathered}\]}
...
\proi{如果基矢$\vec{a},\vec{b},\vec{c}$构成简单正交系,试证明晶面族$(hkl)$的面间距为:$$
        d_{h k l}=\dfrac{1}{\sqrt{\left(\frac{h}{a}\right)^{2}+\left(\frac{k}{b}\right)^{2}+\left(\frac{l}{c}\right)^{2}}}
...
\proi{试思考:为什么晶体衍射实验中要选择波长和晶体中相邻原子间距相当
的波作为入射波?}{}

\proi{试思考:X射线衍射、电子衍射和中子衍射的共性和不同之处?
}{}

\proi{试思考:劳厄衍射方程的物理基础是什么?
}{}

\proi{试思考:当入射波入射到布里渊区边界时会发生什么?
}{}

\proi{试思考:晶体衍射加强的条件有几种典型不同的表述方式,为什么说这些不
同的表述是等价的?
}{}

\proi{试思考:从形式上看,$KCl$非常相似$KBr$,但对$KCl$进行衍射分析时,实验上
观察到和$KBr$相似的面指数全为偶数的衍射峰,但没有观察到面指数全为奇数
的衍射峰,为什么?
}{}

\proi{对正交简单晶格,假设沿三个基矢方向的周期分别为$a、b \text{和}c$的,当入射X
射线方向沿[100]方向(其重复周期为$a$)时,试确定在哪些方向上会出现衍
射极大?什么样的X射线波长才能观察到极大?
}{}

\proi{基于布拉格所给出的晶体衍射加强的条件,试论证只有波长和晶格常数相当
的入射波才有可能发生晶体衍射现象.
}{}

\proi{假设具有立方对称、由同种原子构成的某种晶体,在对其进行X射线分析时,
在衍射谱图中只观察到$(110),(200),(220)\text{或} (222)
\text{等衍射峰,但没有观察到}(100),(300),(111)
\text{或} (221)$
等衍射峰,试通过分析说明该晶体具有何种类型的晶体结构.
}{}

\proi{对面心立方结构的$KBr$晶体,其中$K$和$Br$离子各自组成一套面心
格子,试通过分析论证该晶体的衍射谱图有和特征?
}{}

\proi{对由同种原子(碳)构成的金刚石晶体,试求出衍射强度不为零的条
件.}{}
